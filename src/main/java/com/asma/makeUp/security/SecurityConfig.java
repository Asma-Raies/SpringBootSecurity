...
public class SecurityConfig {
    ...
    public InMemoryUserDetailsManager inMemoryUserDetailsManager(PasswordEncoder passwordEncoder) {
        return new InMemoryUserDetailsManager(
                ...
                User.withUsername("asma").password(passwordEncoder.encode("1234")).roles("USER","AGENT").build(),
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeRequests().requestMatchers("/saveMakeUp").hasAnyRole("ADMIN","AGENT");
        httpSecurity.authorizeRequests().requestMatchers("/listeMakeUp") .hasAnyRole("ADMIN","AGENT","USER");
        httpSecurity.authorizeRequests().requestMatchers("/supprimerMakeUp","/modifierMakeUp","/updateMakeUp") .hasAnyRole("ADMIN");
